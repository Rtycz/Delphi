...
begin
     ...
     if (order.get_status = 0) then begin
         color := RGB(102, 205, 170); //LawnGreen (Green)  Не распределенный
     end
     else if (order.get_status = 2) then begin
          color := RGB(240, 230, 140); //Khaki (Yellow) Не просмотренный
     end
     else if (order.get_status = 3) then begin
          color := RGB(250, 0, 0); //Red (Красный) Просмотренный
     end
     else if (order.get_status = 4) then begin
          color := RGB(0, 0, 255); //Blue (Синий) На месте
     end
     else if (order.get_status = 5) then begin
          color := RGB(200, 200, 200); //Серый Начал исполнение
     end
     else if (order.get_status = 6) then begin
         color := RGB(0, 255, 0); //Green (Зеленый)   Завершенный
     end
     else begin
         color := clPurple;
     end;
